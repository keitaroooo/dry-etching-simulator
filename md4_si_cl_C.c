...
int main()

 {
   ...
   int par1=par1_base+2*unit_num_x*unit_num_y;//基本セル中の原子の数(Si)
   ...
   double a_y=a*unit_num_y;//基本セルのx方向の1辺の長さ
   
   double a_z=a*unit_num_z;//基本セルのx方向の1辺の長さ

   double cut1=unit_num_x*a/2;//SiとClのカットオフ距離[Å](基本セル１辺の長さ/2)
       ...
       for(j=0;j<par1_all;j++){
         ...
         if(r_ij<cut1){
           for(k=0;k<3;k++){
             ...
             f[k]=(2*a1*E1*(exp(-a1*(r_ij-ra1))-exp(-2*a1*(r_ij-ra1))))*(c1[j][k]-c2[i][k])/r_ij;
