...

{
    ...
    int par1_in_unit = 8;
    // x,y,z方向の単位格子の数
    int unit_num_x = 2, unit_num_y = 2, unit_num_z = 2;
    //基本セル中の原子(Si)の数
    int par1 = par1_in_unit * unit_num_x * unit_num_y * unit_num_z;
    //基本セル１つ当たりに近づける原子(Cl)の数
    int par2 = 1;
    //カットオフ距離内の原子の保管用(j)
    int num_j[par1 * 18];
    //カットオフ距離内の原子の保管用(k)
    ...
    //座標［Å］
    double c1[par1 * 18][3], c2[par2 * 9][3];
    //速度［m/s］
    double v1[par1 * 18][3], v2[par2 * 9][3], v;
    //力［N］
    double f1[par1 * 18][3], f2[par2 * 9][3];
    //時間
    double t, dt = 0.5e-15;
    // 1[Å]=1e-10[m]
    double angs = 1e-10;
    // 1[eV]=1.60218e-19[J]
    double ev = 1.60218e-19;
    //温度[K](T0:設定温度)
    ...
    //原子パラメータ
    // Siの原子質量[kg]
    double m1 = 28.0855 / avogadro * 1e-3;
    // Clの原子質量[kg]
    double m2 = 35.453 / avogadro * 1e-3;
    // Siの格子定数[Å]
    double a = 5.4301;
    // SiとClのカットオフ距離[Å]
    double cut1 = 3.5;
    ...
    // Cl
    for(i = 0; i < par2; i++) {
        // par2の個数が多いときは同じ位置には存在しないように注意
        c2[i][0] = a * unit_num_x / 2;
        c2[i][1] = a * unit_num_y / 2;
        //基本格子からカットオフ距離だけ離す
        c2[i][2] = unit_num_z * a + cut1;
    }
    ...
    for(t = 0; t < dt * loop; t += dt, count++) {
        ...
        for(i = 0; i < par1 * 18; i++) {
            ...
            for(j = 0; j < l; j++) {
                ...
                for(k = 0; k < l; k++) {
                    if(num_k[k] != num_j[j]) {
                        ...
                        g = 1 + c * c / d / d -
                            c * c /
                                (d * d +
                                 pow(h - ang(c1, c1, c1, i, num_j[j], num_k[k]),
                                     2));
                        z += fc_ik * g;
                    }
                }
                ...
                // Fi(力)の計算(第1項)
                for(k = 0; k < 3; k++) {
                    f[k] = (-dfc_ij * fr - fc_ij * (-lam1 * fr)) *
                           (c1[i][k] - c1[num_j[j]][k]) / r_ij;  //-lam1*fr=dfr
                    f[k] += -b * (dfc_ij * fa + fc_ij * (-lam2 * fa)) *
                            (c1[i][k] - c1[num_j[j]][k]) / r_ij;
                    f1[i][k] += f[k];
                    f1[num_j[j]][k] -= f[k];
                }
                ...
                for(k = 0; k < l; k++) {
                    if(num_k[k] != num_j[j]) {
                        ...
                        if(R - D < r_ik) {
                            fc_ik =
                                1. / 2. * (1 - sin(pi / 2 * (r_ik - R) / D));
                            dfc_ik =
                                -pi / (4 * D) * cos(pi / 2 * (r_ik - R) / D);
                        } else {
                            fc_ik = 1;
                            dfc_ik = 0;
                        }
                        ...
                        g = 1 + c * c / d / d -
                            c * c / (d * d + pow(h - co, 2));
                        dg = -2 * c * c * (h - co) /
                             pow(d * d + pow(h - co, 2), 2);

                        //∂ζ/∂riの計算(第1項)
                        for(m = 0; m < 3; m++) {
                            dz[m] = g * dfc_ik * (c1[i][m] - c1[num_k[k]][m]) /
                                    r_ik;
                            //∂ζ/∂riの計算
                            dz_i[m] =
                                dz[m] +
                                fc_ik * dg *
                                    ((co / r_ij - 1 / r_ik) *
                                         (c1[num_j[j]][m] - c1[i][m]) / r_ij +
                                     (co / r_ik - 1 / r_ij) *
                                         (c1[num_k[k]][m] - c1[i][m]) / r_ik);
        ...
        for(i = 0; i < par2 * 9; i++) {
            ...
            for(j = 0; j < par1 * 18; j++) {
                ...
                if(r_ij < cut1) {
                    for(k = 0; k < 3; k++) {
                        f[k] =
                            -(-4944.95 + 11219.1 * r_ij -
                              10812.5 * pow(r_ij, 2) + 5724.85 * pow(r_ij, 3) -
                              1794.77 * pow(r_ij, 4) + 332.734 * pow(r_ij, 5) -
                              33.7558 * pow(r_ij, 6) + 1.44548 * pow(r_ij, 7) +
                              0.66648) *
                            (c2[i][k] - c1[j][k]) / r_ij;
        ...
        if(count % 50 == 0) {
            ...
            char filename[100];
            sprintf(filename, "MD%06d.txt",
                    count / 50);  // MD%06d.txtという文字列を配列filenameに挿入
            outputfile = fopen(
                filename,
                "w");  //ファイルを書き込み用にオープン(MD%06d.txtというファイルを開く)
            ...
            for(i = 0; i < par1; i++) {
                fprintf(outputfile, "%d 2 %lf %lf %lf\n", i, c1[i][0], c1[i][1],
                        c1[i][2]);  // MD%06d.txtというファイルに書く
            }

            // for(i=par1;i<par1*18;i++){

            // fprintf(outputfile,"%d 3 %lf %lf
            ...
            for(i = 0; i < par2; i++) {
                fprintf(
                    outputfile, "%d 0 %lf %lf %lf\n", i, c2[i][0], c2[i][1],
                    c2[i]
                      [2]);  // MD%06d.txtというファイルに書く(cl),[通し番号][粒子種][x座標][y座標][z座標]
            }

            //   for(i=par2;i<par2*9;i++){
            // fprintf(outputfile,"%d 0 %lf %lf
...
double ang(double atm_i[][3], double atm_j[][3], double atm_k[][3], int i,
           int j, int k) {
