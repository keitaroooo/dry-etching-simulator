...
int main()

 {
   ...
   int par2=10;//基本セル１つ当たりに近づける原子(Cl)の数
   ...
   double m2=18.9984/avogadro*1e-3;//Fの原子質量[kg](18.9984:原子量)
   ...
   double lam1=3.2394;//[Åe-1]
  
   double lam2=1.3258;//[Åe-1]
  
   double beta=3.3675e-1;
   ...
   double A=3.2647e+3;//[eV]

   double B=9.5373e+1;//[eV]
   ...
   double cut1=4.5;//SiとFのカットオフ距離[Å]
       ...
       for(j=i+1;j<par2_all;j++){
         ...
         if(r_ij<cut2){
           for(k=0;k<3;k++){
             ...
             f[k]=-(2*a2*E2*(exp(-a2*(r_ij-ra2))-exp(-2*a2*(r_ij-ra2))))*(c2[i][k]-c2[j][k])/r_ij;//-∂U/∂r*(c2i-c2j)/rでj→i方向が正

             f2[i][k]+=f[k];//(斥力が働いていてi原子のほうが正方向にあるときf2[i][k]>0)
             f2[j][k]-=f[k];
	   }
	 }
       }
